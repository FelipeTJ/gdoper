...
from src.d_print import Debug, Stats, Set_PrintLevel
...
def batch_process(in_dir, out_dir, func, stats=True):
  ...
  if stats:
    print()
    Stats(f'Process:{func.__name__}')
    Stats(f'Total processing time: {end-start:.2f}s')
...
def flight_time_counter(in_file, in_dir, out_dir):
  ...
  data = r.get_merged_cols(CHN_UTC)
  
  t0 = dt.datetime.fromisoformat(data[CHN_UTC][0])
  t1 = dt.datetime.fromisoformat(data[CHN_UTC][-1])
...
def sats2gdop_getter(in_file, in_dir, out_dir, d):
  ...
  data = r.get_merged_cols(CHN_SAT, 'GDOP', 'VDOP', 'HDOP')
...
def get_batch_stats(f_in, print_lvl, plot=False):
  global mean_speed
  global max_speed
  global tot_time
  ...
  batch_process(f_in, BASE_FOLDER + SE + 'Tampere_plots', flight_time_counter, stats=False)
  batch_process(f_in, BASE_FOLDER + SE + 'Tampere_plots', speed_counter, stats=False)

  if plot:
    plt.clf()
    plt.cla()
    plt.plot(sorted(mean_speed))
    plt.show()
