...
from reader_pos_data import PosData
from reader_rinex import OrbitalData
import d_print as p
from common import *
...
class FOV_simple:
    ...
    def __init__(self, u) -> None:
        self.m_a = math.pi * (LOS_ANGLE) / 180  # Mask angle in radians
...
class Gdop:
    def __init__(self, filename: str, output_file: str, fov: str = "", t_s: float = 5):
        ...
        self.fov_model = fov
        self.T_s = timedelta(seconds=t_s)  # Sampling period

        self.pos_index = 0
        self.pos_obj = PosData(filename)
        self.pos_data = self.pos_obj.get_merged_cols(CHN_DEFAULTS)  # TODO: Change to dataset

        # print(self.pos_data)
        self.sat_data = OrbitalData(self.pos_data[CHN_UTC][self.pos_index])
        self.sat_poss = self.sat_data.get_sats_pos([self.pos_obj.get_first_utc()])  # Deprecated
        self.measured_visible_sats = self.pos_data[CHN_SAT][self.pos_index]
    ...
    def lla2ecef_drone(self) -> tuple:
        ...
        lat = self.pos_data[CHN_LAT][self.pos_index]
        lon = self.pos_data[CHN_LON][self.pos_index]
        alt = self.pos_data[CHN_ALT][self.pos_index]
    ...
    def get_all_gdop(self):
        ...
        p.Print(
            "info",
            f"{p.GREEN}Calculating GDOP{p.CEND} ({self.pos_obj.row_count} rows,"
            f" calculate every {GDOP_INTERVAL.seconds}s)",
        )
        ...
        for i in range(1, self.pos_obj.row_count):
            self.pos_index = i
            new_date = datetime.fromisoformat(self.pos_data[CHN_UTC][self.pos_index])
            if new_date - self.sat_data.utc < self.T_s:
                p.Print(
                    "debug0",
                    f"{p.VIOLET}new_date is too close ({new_date-self.sat_data.utc}){p.CEND}",
                )
                gdops.append(0)
                inview.append(0)
                continue
