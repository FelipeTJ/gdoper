...
class Calc_manager:
    def __init__(
        ...
        # Objects
        self.pos_obj = PosData(str(self.pdata_dir) + self.input_file)
        self.sat_obj = OrbitalData()
    ...
    def __setup(self) -> None:
        ...
        if self.fov_obj == FOV_model():
            raise Exception(
                "FOV_model is the base class, it cannot be used.\
                      Select an inherited model."
            )

        if len(self.calcs_q) == 0:
            raise Exception(
                "No calculations have been queued.\
        ...
        for i in self.calcs_q:
            if i == Calc():
                raise Exception(
                    "Calc cannot be used as a calculation.\
    ...
    def process_data(self):
        ...
        Debug(
            f"Total runtime: {time.perf_counter() - tot:.3f}"
